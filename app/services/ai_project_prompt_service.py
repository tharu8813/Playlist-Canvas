...
- `playlist` 항목 필수 키: `file_path`, `title`. 선택 키: `id`, `artist`, `album`, `duration_seconds`, `start_time_seconds`, `enabled`, `lyrics_path`, `lyrics`, `lyrics_timing_offset_seconds`. `lyrics`는 `[{"start": 시작초, "end": 종료초, "text": "가사"}]` 배열로 만들며, 곡별 가사 보정은 초 단위의 `lyrics_timing_offset_seconds`에 넣는다.
...
- Every `playlist` item requires `file_path` and `title`; optional keys are `id`, `artist`, `album`, `duration_seconds`, `start_time_seconds`, `enabled`, `lyrics_path`, `lyrics`, and `lyrics_timing_offset_seconds`. Use `[{"start": start_seconds, "end": end_seconds, "text": "line"}]` for inline lyrics and seconds for the per-track timing offset.
