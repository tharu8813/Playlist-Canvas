...
- 가사 키: `subtitle_fallback`, `subtitle_style`(`karaoke|minimal|neon`), `subtitle_animation`(`glow|rise|none`), `subtitle_animation_duration`, `subtitle_context_lines`, `subtitle_next_lines`, `subtitle_line_spacing`, `subtitle_previous_opacity`, `subtitle_previous_blur`, `subtitle_timing_offset`. 별도 요구가 없으면 부드러운 `glow`를 우선 사용한다.
...
- Lyrics keys: `subtitle_fallback`, `subtitle_style` (`karaoke|minimal|neon`), `subtitle_animation` (`glow|rise|none`), `subtitle_animation_duration`, `subtitle_context_lines`, `subtitle_next_lines`, `subtitle_line_spacing`, `subtitle_previous_opacity`, `subtitle_previous_blur`, `subtitle_timing_offset`. Prefer the smooth `glow` option unless another transition is requested.
