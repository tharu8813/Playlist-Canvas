...
`dist\Playlist Canvas` 폴더 전체를 압축하거나 설치 프로그램에 포함해야 합니다. 실행 파일만 따로 복사하면 Qt 및 Python 런타임 파일이 없어 실행되지 않습니다.
...
1. `Playlist Canvas.exe`가 실행되는지 확인합니다.
2. 설정 화면에서 언어·테마·출력 폴더를 저장하고 재시작 후 유지되는지 확인합니다.
3. 설정 화면의 FFmpeg 다운로드를 사용해 설치와 SHA-256 검증이 완료되는지 확인합니다.
4. MP3와 이미지 파일을 드래그 앤 드롭하여 MP4 내보내기가 정상 동작하는지 확인합니다.
5. `%LOCALAPPDATA%\PlaylistCanvas\logs\playlist-canvas.log`에 오류 로그가 생성되는지 확인합니다.
